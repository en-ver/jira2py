...
`helpers.metadata.issue_types()` aggregates all valid create-issue-type pages into an ordered bare list of raw mappings. `create_fields()` scans issue-type pages in Jira order until the first case-insensitive match, stops that lookup immediately, then aggregates all valid create-field pages for the matched ID. Both preserve raw mappings and unknown item properties without returning a page envelope. Canonical `values` pages require valid `startAt` and `isLast` metadata; metadata-free `issueTypes` or `fields` fallbacks are compatibility-terminal, while signaled fallback pagination must be complete. Use the low-level `api.issues.get_create_issue_types()` and `api.issues.get_create_fields()` with `start_at`/`max_results` only when you need individual raw page envelopes.
