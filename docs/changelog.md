...
- Managed Jira Markdown images now receive exact intrinsic dimensions before a helper
  writes ADF. A 64-byte authenticated no-follow range probe handles PNG/APNG, GIF,
  WebP, and BMP first; JPEG, TIFF, and inconclusive probes use one complete fallback
  limited to 100 MiB. JPEG/TIFF EXIF orientation is applied. Dimensions must be
  positive, at most 65,535 per axis, and at most 89,478,485 pixels. The resulting
  managed media is centered at 100% parent width with intrinsic child dimensions.

### Compatibility

- Public attachment downloads retain their existing redirect-following, fully buffered
  behavior. Managed-image acquisition remains private, avoids external fetches, and
  verifies raw persisted ADF structure rather than making a browser-rendering claim.

### Security fixes

- Private managed-image reads use `redirect=false`, do not follow redirects, request
  identity encoding, and validate the response status, byte range, total size, and
