...
The top-level imports below are jira2py's custom low-level HTTP, transport, and attachment-protocol error hierarchy. `JiraError` is not a catch-all for every exception a jira2py call can raise; high-level helper errors are independent. See [Helper errors](https://jira2py.org/api/helpers/#helper-errors) for that hierarchy.
